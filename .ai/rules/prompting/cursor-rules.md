...
A comprehensive meta-prompt for creating effective, modular, and well-structured Cursor Rules files that maximize AI agent performance and development productivity.

## Meta-Prompting Framework

### Structure-Oriented Design Principles
Following meta-prompting research (Zhang et al., 2024), this guide emphasizes **structure and syntax over content**, enabling:
...
- **Always Applied** (`alwaysApply: true`): Core standards that apply universally
- **Auto Attached** (`globs: pattern`): Domain-specific rules triggered by file patterns  
- **Agent Requested** (`description` required): Rules AI can choose to apply based on context
...
This rule ensures [specific outcome] by providing [type of guidance].
Use when working with [specific context/files/situations].
```
...
- **Flexible**: Use conditional language ("when X, then Y") for context-dependent advice
...
- Provide specific criteria for decision-making (e.g., "functions > 20 lines", "coverage < 80%")

### Structured Instructions
- Group related concepts together
- Use consistent formatting for similar types of information
- Employ delimiters (code blocks, tables, lists) to separate different content types
...
**Structure**: [SYNTAX_PATTERN]
**Application**: [WHEN_TO_USE]
...
- **Clear dependencies**: Reference other rules or files explicitly using `@filename`

### Maintainability Features
- **Version awareness**: Reference specific tool/language versions when relevant
- **Update triggers**: Note when the rule should be reviewed (e.g., "Review when upgrading to React 19+")  
...
```
[DOMAIN]_PATTERN := [TYPE_A] | [TYPE_B] | [TYPE_C]
[DOMAIN]_SCOPE := [SCOPE_OPTIONS]
[DOMAIN]_COMPLEXITY := [COMPLEXITY_LEVELS]
```
...
- **For applications**: Include user-friendly error messages and recovery options
...
**Reason**: [ANALYZE_THE_SITUATION]
**Act**: [SPECIFIC_IMPLEMENTATION_STEPS]
**Observe**: [VALIDATION_AND_FEEDBACK]
**Reflect**: [LESSONS_AND_IMPROVEMENTS]
```
...
**Issue**: AI agents may be unable to add or modify MDC frontmatter due to software limitations with YAML parsing in edit operations.
...
**Resolution**:
1. **Agent should specify the required frontmatter** in clear format for manual addition
...
```markdown
### [PATTERN_NAME]
**Syntax**: [ABSTRACT_STRUCTURE]
**Variables**: [PARAMETER_TYPES]
...
- **Performance data**: Adjust based on metrics like build times, error rates, test coverage
- **Prompting research**: Integrate new techniques from meta-prompting and AI research
...
Remember: The best rules are those that become invisible—they guide behavior so naturally that following them feels automatic. They should be **structure-oriented**, **token-efficient**, and **categorically organized** to maximize AI agent performance.
